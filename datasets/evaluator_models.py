...
class BERT(nn.Module):
    ...
    def compute_loss(self, batch):
        ...
        mixed_bert_loss, bert_losses = self.compute_bert_losses(batch)
        losses.update(bert_losses)
        losses["total"] += mixed_bert_loss
    ...
    def compute_bert_losses(self, batch):
        mixed_bert_loss = 0.
        bert_losses = {}

        if 1:
            for d in self.bert_training.split('_')[:1]:
                if d == 'image':
                    features = self.bert_model.encode_image(batch['images']).float()  # preprocess is done in dataloader
                elif d == 'text':
                ...
                bert_mixed_loss = (ce_from_motion_loss + ce_from_d_loss) / 2.

                bert_losses[f'{d}_ce_from_d'] = ce_from_d_loss.item()
                bert_losses[f'{d}_ce_from_motion'] = ce_from_motion_loss.item()
                bert_losses[f'{d}_mixed_ce'] = bert_mixed_loss.item()
                mixed_bert_loss += bert_mixed_loss

        return mixed_bert_loss, bert_losses
